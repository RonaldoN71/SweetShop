...
export default function SweetList() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (!q.trim()) {
        ...
        setFiltered(sweets.filter((s) => s.name.toLowerCase().startsWith(lower)));
      }
    }, 180);
  ...
  return (
    <DashboardLayout
      ...
      actions={
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full">
          ...
          />
          {isAdmin() && (
            <Button variant="primary" className="sm:w-auto w-full" onClick={() => navigate("/create")}>
              + Create Sweet
            </Button>
          )}
        </div>
    ...
    >
      {loading && (
        <div className="mb-4 text-sm text-blue-700 bg-blue-100 p-2 rounded">Loading…</div>
      )}
      {err && (
        <div className="mb-4 text-sm text-red-700 bg-red-100 p-2 rounded">{err}</div>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full p-6 bg-white rounded-lg border">No sweets found.</div>
        ) : (
          filtered.map((s) => (
            <SweetCard key={s._id} sweet={s} isAdmin={isAdmin()} onDelete={handleDelete} />
          ))
        )}
      </div>
    </DashboardLayout>
